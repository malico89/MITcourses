...
class WeightedEdge(Edge):
    def __init__(self, src, dest, total_distance, outdoor_distance):
        self.src = src
        self.dest = dest
        self.dist = int(total_distance)
        self.odist = int(outdoor_distance)
    ...
    def __str__(self):
        return "{}->{} ({}, {})".format(self.src, self.dest, self.dist, self.odist)
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        else:       # add to node set and edges dict
        # hash the node to use as key. Value is empty list (to be filled with edge obj)    
            self.edges[hash(node)] = []
            self.nodes.add(node)
    ...
    def add_edge(self, edge):
        ...
        if edge.get_source() not in self.nodes or edge.get_destination() not in self.nodes:
            raise ValueError('Source or Destination nodes not in graph.')
        else:
            # hash the node object to use as key. Value is a list of edge objects.
            self.edges[hash(edge.get_source())].append(edge)
